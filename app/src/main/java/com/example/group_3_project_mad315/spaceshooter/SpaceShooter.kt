...
class SpaceShooter(context: Context) : View(
    context
) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        canvas.drawText("Score: $points", 10f, TEXT_SIZE.toFloat(), scorePaint)
        ...
        canvas.drawBitmap(
            enemySpaceship.getEnemySpaceship(),
        ...
        for (i in enemyShots.indices-1) {
            ...
            if (enemyShots[i].shx >= ourSpaceship.ox
                && enemyShots[i].shx <= ourSpaceship.ox + ourSpaceship.getOurSpaceshipWidth() && enemyShots[i].shy >= ourSpaceship.oy && enemyShots[i].shy <= screenHeight
            ) {
                life--
        ...
        for (i in ourShots.indices) {
            ...
            if (ourShots[i].shx >= enemySpaceship.ex
                && ourShots[i].shx <= enemySpaceship.ex + enemySpaceship.getEnemySpaceshipWidth() && ourShots[i].shy <= enemySpaceship.getEnemySpaceshipWidth() && ourShots[i].shy >= enemySpaceship.ey
            ) {
                points++
                ourShots.removeAt(i)
        ...
        // If not paused, we’ll call the postDelayed() method on myhandler object which will cause the
        // run method inside Runnable to be executed after 30 milliseconds, that is the value inside
        // UPDATE_MILLIS.
        if (!paused) myhandler!!.postDelayed(runnable, UPDATE_MILLIS)
    ...
    init {
        ...
        myhandler = Handler()
        background = BitmapFactory.decodeResource(context.resources, R.drawable.spaceshooter_background)
        lifeImage = BitmapFactory.decodeResource(context.resources, R.drawable.spaceshooter_life)
        scorePaint = Paint()
